...
public class AuthSmsCodeServiceImpl implements AuthService, Serializable {
	...
	public AuthSmsCodeServiceImpl(LogService logService, FactoryService iresource, AuthServiceManager authServiceManager, CacheServiceManager cacheServiceManager) {
		logger = logService.getLogger(this.getClass().getName());
		this.factoryService = iresource;
		this.authServiceManager = authServiceManager;
		this._cacheServiceManager=cacheServiceManager;
	}
	...
	public Online login(Map per) {
		...
		if (!StringUtil.isEmptys(username) && !StringUtil.isEmptys(password)) {
			...
			Object smscode = _cacheServiceManager.get("_system_phone_code_", username);
			if (smscode != null && smscode.equals(password)) {
