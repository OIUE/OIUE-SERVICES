...
public class PostServlet extends HttpServlet {
	...
	private ConvertServiceManager convertService;
	private Logger logger;
	private Map properties;
	private boolean refererGrant = false;
	private static long timeout = 5000;

	public PostServlet(ActionService actionService, LogService logService, ConvertServiceManager convertService) {
		super();
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		TaskThread t = null;
		try {
			String c_referer = MapUtil.getString(properties, "Referer");
			if (refererGrant) {
			...
			if ("csv".equals(export)) {
				...
				per.put(DriverDataField.driverName, MapUtil.getString(per, "modulename"));
				per.put(DriverDataField.type, MapUtil.getString(per, "operation"));
				StatusResult afr =convertService.convert(per);
