...
public class ActionServiceImpl implements ActionService {
	...
	public Map request(Map per) {
		...
		try {
			...
			Online online = (Online) per.remove(PermissionConstant.permission_user_key);
			...
			if(afterActionFilter.values().size()>0){
				String source_data_str = (data instanceof Map)?JSONUtil.parserToStr((Map)data):(data instanceof List)?JSONUtil.parserToStr((List)data):"";
