...
public class WebSocketServerImpl implements WebSocketService, HttpService {
	...
	public WebSocketServerImpl(ActionService actionService) {
		super();
		// this.host = host;
		// this.port = port;
	...
	public void start() {
		...
		sb.group(bossGroup, workerGroup).channel(NioServerSocketChannel.class).childHandler(new ChannelInitializer<Channel>() {
			@Override
	...
	@Override
	public void handleHttpRequset(ChannelHandlerContext ctx, FullHttpRequest request) {
	...
	private boolean isWebSocketUpgrade(FullHttpRequest request) {
		...
		return request.method().equals(HttpMethod.GET) && headers.get(HttpHeaderNames.UPGRADE).contains(WEBSOCKET_UPGRADE) && headers.get(HttpHeaderNames.CONNECTION).contains(WEBSOCKET_CONNECTION);
	}
}